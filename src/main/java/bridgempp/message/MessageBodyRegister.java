...
public class MessageBodyRegister
{
	...
	public static <I extends MessageBody, O extends MessageBody> O convert(I messageBody, Class<O> targetClass)
	{
		...
		{
			Log.log(Level.WARNING, "Failed to convert Message from Format: " + messageBody.getClass().getSimpleName() + "\nMessage: " + messageBody.toString(), e);
		}
		return null;
	}
	...
	{
		Log.log(Level.INFO, "Building conversion map for all message Formats");
		...
		conversions.forEach((origin, map1) -> {
			...
			Log.log(Level.INFO, "The conversion map for " + origin.getSimpleName() + " has been built:\n" + builder.toString());
		});
	}
	...
	public static void initializeConversionSystem()
	{
		...
		registerConversion(ImageMessageBody.class, XHTMLXMPPMessageBody.class, DEFAULT_COST * 2, t -> {
			try
			{
				return new XHTMLXMPPMessageBody("<img src=\"data:" + t.getMimeType().toString() + ";base64," + t.getDataAsBase64() + "\"/>");
			} catch (IOException e)
			{
