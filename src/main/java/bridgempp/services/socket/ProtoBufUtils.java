...
public class ProtoBufUtils
{
	...
	public static MessageBuilder parseMessage(bridgempp.services.socket.protobuf.Message protoMessage, MessageBuilder messageBuilder)
	{
		protoMessage.getDestinationGroupsList().forEach(group -> messageBuilder.addDestinationsFromGroupNoLoopback(DataManager.getGroup(group.getName())));
		...
		protoMessage.getMessageBodiesList().forEach(messageBody -> {
			switch (messageBody.getMessageType())
			{
				...
				case IMAGE_URL:
					try
					{
						URL imageURL = new URL(messageBody.getContents());
						messageBuilder.addMessageBody(ImageMessageBody.fromURL(imageURL));
					} catch (Exception e)
					{
						Log.log(Level.WARNING, "Could not decode Image URL");
					}
					break;
				...
				case UNRECOGNIZED:
				default:
					Log.log(Level.WARNING, "Got unrecognized Message Type!");
					break;
			}
		});
