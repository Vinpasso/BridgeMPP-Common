...
public class DataManager
{
	...
	private static synchronized Endpoint registerTimedEndpoint(BridgeService service, String identifier, long expiry, TemporalUnit unit)
	{
	 ...
	 * 
	 * @param identifier
	 *            The Primary Key to find the User from
	 * @return The User found from the Primary Key
	 */
	public static synchronized User getUserForIdentifier(String identifier)
	{
		return PERSISTANCE_MANAGER.getFromPrimaryKey(User.class, identifier);
	}

	/**
	 * Fetches a User for a given Identifier. Creates a new User if it doesn't
	 * exist
	 * 
	 * @param identifier
	 *            The Primary Key to find the User from
	 ...
	 * 
	 * @param identifier
	 *            The Primary Key to find the Endpoint from
	...
	{
		return PERSISTANCE_MANAGER.getFromPrimaryKey(Endpoint.class, identifier);
	}

	/**
	 * Fetches a Endpoint for a given Identifier. Creates a new Endpoint if it
	 * doesn't exist
	 * 
	 * @param identifier
	 *            The Primary Key to find the Endpoint from
	...
	{
		return PERSISTANCE_MANAGER.getQuery(Endpoint.class);
	}
	...
	{
		return PERSISTANCE_MANAGER.getFromPrimaryKey(Group.class, name);
	}

	public static synchronized Collection<Group> getAllGroups()
	{
		return PERSISTANCE_MANAGER.getQuery(Group.class);
	}
	...
	public static synchronized Collection<StatisticStore> getStatisticsStore()
	{
		return PERSISTANCE_MANAGER.getQuery(StatisticStore.class);
	}

	public static synchronized void saveStatisticsStore(StatisticStore statisticStore)
	{
		updateState(statisticStore);
	}

	public static synchronized Collection<BridgeService> getServiceConfigurations()
	{
		return PERSISTANCE_MANAGER.getQuery(BridgeService.class);
	}

	public static synchronized Collection<Endpoint> getEndpoints()
	{
		return PERSISTANCE_MANAGER.getQuery(Endpoint.class);
	}
